...
// var sy = arcWidth / 2;
// var ar = half - sy;
// var ex = 0;// ar * Math.sin(Math.PI / 3); // 60°
// var ey = 2 * ar; // ar * Math.cos(Math.PI / 3); // 60°

var sx = arcWidth / 2; // half;
var sy = half;         // arcWidth / 2;
var ar = half - sx;
var ex = ar * 2; // ar * Math.sin(Math.PI / 3); // 60°
var ey = 0;    // ar * Math.cos(Math.PI / 3); // 60°

var pathsCovered = [
  'environment.wind.angleApparent',
  'environment.wind.speedApparent',
  'environment.current.angleTrueGround',
  'environment.current.speedTrue'
]
...
function CurrentMeter(id, options, streamBundle, instrumentPanel) {
  options.unit = 'm/s';
  ...
  //  alert('CurrentMeter ctor called');
}

util.inherits(CurrentMeter, BaseWidget);

CurrentMeter.prototype.handleNewSource = function(newSource) {
...
CurrentMeter.prototype.findSourceForPath= function(path) {
...
CurrentMeter.prototype.combineValueStream = function(displayModeIndex) {
  ...
  if(displayModeIndex === displayApparent) {
    paths = [
      'environment.current.angleApparent',
      'environment.current.speedApparent'
    ];
    this.options.label = 'Apparent Current [AF]';
  } else if(displayModeIndex === displayTrueGround) {
    paths = [
      'environment.current.angleTrueGround',
      'environment.current.speedOverGround'
    ];
    this.options.label = 'True Current over Ground [AF]';
  } else if(displayModeIndex === displayTrueWater) {
    paths = [
      'environment.current.angleTrueWater',
      'environment.current.speedTrue'
    ];
    this.options.label = 'True Current through Water [AF]';
  } else {
    this.options.label = '???';
  }
...
CurrentMeter.prototype.validateOptions = function(options) {
  options.unit = 'm/s';
  return options;
}

CurrentMeter.prototype.metaToOptions = function(pathMeta) {
// ToDo !!!!!!
  return {};
}
...
CurrentMeter.prototype.getReactElement = function() {
  return this.widget;
}

CurrentMeter.prototype.getType = function() {
  return "currentmeter";
}

CurrentMeter.prototype.getHandledSources = function() {
  return Object.values(this.options.activeSources || {});
}

CurrentMeter.prototype.getOptions = function() {
  this.options.unit = 'm/s';
  return this.options;
}

CurrentMeter.prototype.getInitialDimensions = function() {
  return {w:2, h:4};
}
...
CurrentMeter.prototype.getSettingsElement = function(pushCellChange) {
  return this.getSettingsElementUnitOnly(pushCellChange);
}

export default {
  constructor: CurrentMeter,
  type: 'windmeter', // TODO: fix this!!! change to currentmeter or something
  paths: pathsCovered
}
